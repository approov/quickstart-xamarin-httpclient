...
```C#
ApproovService.Initialize(string config)
```
...
```C#
ApproovService.CreateHttpClient(HttpMessageHandler handler)
```
...
```C#
ApproovService.SetProceedOnNetworkFailure(bool proceed)
```
...
```C#
ApproovService.SetTokenHeaderAndPrefix(string header, string prefix)
```
...
```C#
ApproovService.SetBindingHeader(string header)
```
...
```C#
ApproovService.AddSubstitutionHeader(string header, string requiredPrefix)
```
...
```C#
ApproovService.RemoveSubstitutionHeader(string header)
```
...
```C#
ApproovService.AddSubstitutionQueryParam(string key)
```
...
```C#
ApproovService.RemoveSubstitutionQueryParam(string key)
```
...
```C#
ApproovService.AddExclusionURLRegex(string urlRegex)
```
...
```C#
ApproovService.RemoveExclusionURLRegex(string urlRegex)
```
...
```C#
ApproovService.SetDataHashInToken(string data)
```
